...
function ajouterPanier(event, item, elementHeure, jourJour) {
  ...
  const existingCourseAtSameTime = basket.value.find((course) => {
    return course.horaires.some(
      (horaire) => horaire.jour === jourJour && horaire.heure === elementHeure
    )
  })
        ...
        <caption>
          <div class="captionTable">
            <div class="contenuCentre">
              <span class="sessionEmploi">SESSION DECEMBRE</span> 2022
            </div>
          ...
          <tr v-for="element in coursesTable" :key="element.heure">
            ...
            <td class="tdCours" v-for="jour in element.coursParJour" :key="jour.jour">
              <div
                class="celluleCours"
                style=""
                v-for="crs in jour.cours"
                :key="crs.id"
                :id="element.heure + '-' + jour.jour + '-' + crs.id"
                :data-cle="crs.id"
                @click="ajouterPanier($event, crs, element.heure, jour.jour)"
              >
                {{ crs.name }}
              </div>
            </td>
